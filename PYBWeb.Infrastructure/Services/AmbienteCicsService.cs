    ...
    public async Task<AmbienteCics? > ObterPorNomeAsync(string idChave)
{
    ...
    public async Task<bool> AtualizarAmbienteAsync(AmbienteCics ambiente)
    {
        try
        {
            ...
            ambienteExistente.Pxu = ambiente.Pxu;
...
public async Task<bool> AlterarStatusPxuAsync(int id, int pxu)
{
    try
    {
        using var context = new AmbienteDbContext(ObterConnectionString());
        ...
        if (ambiente == null)
        {
            _logger.LogWarning("Ambiente não encontrado para alteração de status PXU: {Id}", id);
            return false;
        }
        ...
        _logger.LogInformation("Status PXU do ambiente alterado: {Id} - {Nome} -> {Pxu}", id, ambiente.Nome, pxu);
        return true;
    }
    catch (Exception ex)
    {
        _logger.LogError(ex, "Erro ao alterar status PXU do ambiente: {Id}", id);
        return false;
    }
}

public async Task<List<AmbienteCics>> ObterAmbientesPxuAtivosAsync()
{
    try
    {
        using var context = new AmbienteDbContext(ObterConnectionString());
        ...
        return await context.Ambientes
            .Where(a => a.Ativo && a.Pxu == 1)
            .OrderBy(a => a.Nome)
            .ToListAsync();
    }
    catch (Exception ex)
    {
        _logger.LogError(ex, "Erro ao obter ambientes PXU ativos");
        return new List<AmbienteCics>();
    }
}
}